...
export default function PlusBtn({ onHighlight }) {
    ...
    return (
        <div className='plus-container'>
            ...
            <button onClick={() => handleSubjectClick('미시경제학', ['월 - 1~2시', '수 - 11~1시', '목 - 9~11시'])}>
                <h3><nav className='required'>필수</nav> - 미시경제학 (제니퍼)</h3>
            </button>
            <br />
            <button onClick={() => handleSubjectClick('노동경제학', ['수 - 1~3시', '목 - 4~6시', '금 - 10~12시'])}>
                <h3><nav className='elective'>선택</nav> - 노동경제학 (김유신)</h3>
            </button>
            <button onClick={() => handleSubjectClick('환경경제학', ['목 - 7~8시', '금 - 1~2시', '화 - 9~10시'])}>
